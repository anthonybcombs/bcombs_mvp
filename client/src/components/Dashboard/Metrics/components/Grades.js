...
const apiCallGrades = async (vendorId, id, year, grade) => {
...
const Mentoring = props => {
    ...
    useEffect(() => {
        if (auth && auth.user_id) {
            triggerApiCallGrades(auth.user_id, year, grade);
        }
    }, [auth]);

    const triggerApiCallGrades = async (id, year, grade) => {
        try {
            ...
            const res = await apiCallGrades(vendorId, id, year, grade);
