...
const Mentoring = props => {
    ...
    useEffect(() => {
        if (auth && auth.user_id) {
            triggerApiCallMentoring(auth.user_id, year, grade);
        }
    }, [auth]);
    ...
    const yearChange =(event) => {
        ...
        triggerApiCallMentoring(auth.user_id, event.target.value, grade);
    };
    ...
    const gradeChange =(event) => {
        ...
        triggerApiCallMentoring(auth.user_id, year, event.target.value);
    };
