...
export default (props) => {
  const { label, id = '', onChangeFieldSettings, isBuilder, pageBreaks, isLastPageBreak, value = '' } = props
  const pageOptions = [
  ...
  return (
    <div className='field select-field-wrapper' style={{ display: 'flex', alignItems: 'center' }}>
      {
        (isBuilder && !isLastPageBreak) && (
          <>
            <div style={{ paddingRight: '10px' }}>After Page {pageBreakIndex + 1} ({label})</div>
            <select
              value={value}
