...
export const removeContact = async id => {
  ...
  try {
    await db.query(`DELETE FROM contacts WHERE id=UUID_TO_BIN(?)`, [id]);
      await db.query(`DELETE FROM group_members WHERE user_id=UUID_TO_BIN(?)`, [id]);
    const rows = await db.query(
      `SELECT BIN_TO_UUID(id) as id,first_name,last_name,email,phone_number  from contacts`
    );
    result = rows;
...
export const editContact = async data => {
  ...
  try {
    ...
    await db.query(`UPDATE contacts SET first_name=?,last_name=?,phone_number=?,email=?,relation=? WHERE id=UUID_TO_BIN(?)`, [
