...
const getAverage = (grades, type) => {
  return grades.map((grade) => {
    if (type === "semestral") {
      return {
        ...grade,
        semestral_1_average: parseFloat(
          ((grade.grade_quarter_1 || 0) + (grade.grade_quarter_2 || 0)) / 2
        ),
        semestral_2_average: parseFloat(
          ((grade.grade_quarter_3 || 0) + (grade.grade_quarter_4 || 0)) / 2
        ),
      ...
      return {
        ...
        attendance_quarter_1_present:
          grade.attendance_quarter_1_total -
          (grade.attendance_quarter_1_tardy +
            grade.attendance_quarter_1_absent),
        attendance_quarter_2_present:
          grade.attendance_quarter_2_total -
          (grade.attendance_quarter_2_tardy +
            grade.attendance_quarter_2_absent),
        attendance_quarter_3_present:
          grade.attendance_quarter_3_total -
          (grade.attendance_quarter_3_tardy +
            grade.attendance_quarter_3_absent),
        attendance_quarter_4_present:
          grade.attendance_quarter_4_total -
          (grade.attendance_quarter_4_tardy +
            grade.attendance_quarter_4_absent),
      };
    }
    return {
      ...grade,
...
export const getStudentCumulativeByChildId = async (childId, type = 'bcombs') => {
  ...
  try {
    ...
    if (response) {
      ...
      const studentGradeCumulativeIds = response.map(
        (item) => item.student_grade_cumulative_id
      );
...
export const getStudentCumulativeGradeByAppGroupId = async (
  app_group_id,
  app_group_type = "application"
) => {
  const db = makeDb();
  let studentCumulative = [];
  let studentGrades = [];
  ...
  try {
    ...
    if (response && response[0]) {
      ...
      if (studentCumulative) {
        ...
        studentCumulative.grades = getAverage(
          studentCumulative.grades,
          studentCumulative.school_year_frame
        );
      }
    }
  } catch (error) {
...
export const addUpdateStudentCumulativeGrades = async (
  ...
  try {
      ...
      if (isUserExist.length === 0) {
        ...
        `,
          [
            ...
            s3Payload
              ? s3Payload.Key
              : isUserExist[0] && isUserExist[0].attachment,
            child_id,
            cumulativeId,
          ]
        );
    ...
    await db.close();
    console.log(
      "studentCumulativeGradesResult 111",
      studentCumulativeGradesResult
    );
  ...
  try {
    for (const test of studentTest) {
      ...
      if (!isTestExist && !studentTestId) {
        ...
        const s3Payload = await formatFile(
          test.attachment,
          studentTestId,
          "student_test"
        );
        ...
        `,
          [
            ...
            s3Payload
              ? s3Payload.Key
              : isTestExist[0] && isTestExist[0].attachment,
            studentTestId,
          ]
        );
      }
